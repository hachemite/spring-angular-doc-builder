...
export class AuthService {
  ...
  // CHANGED: Use environment variable + specific endpoint
...
export const routes: Routes = [
  ...
  },
  {
    ...
    canActivate: [authGuard] // Ensure only admins can access in real app
  },
  {
    path: 'document/:id',
...
export class DocumentService {
  private http = inject(HttpClient);
  private apiUrl = 'http://localhost:8080/api/documents'; // Adjust based on env

  generateDocument(data: any): Observable<string> {
    // The backend returns a text response ("Document generated...")
    // so we set responseType: 'text'
    return this.http.post(`${this.apiUrl}/generate`, data, { responseType: 'text' });
  }
...
export class App {
  ...
  // Helper to check login status for the HTML template
  ...
  logout() {
    ...
    // The service handles clearing token and navigating to login
  }
}
